<!DOCTYPE html>
<html lang="en">
<head>
    <?php require_once './elements/head.php'; ?>
    <title>Hai Phong City - Travel</title>
</head>
<body>
    <?php require_once './elements/header.php'; ?>
    <div class="peopleContact">
        <div class="peopleInformationWrapper">
            <div class="titleIntroductionWrapper">
                <h1 class="titleIntroductionMain">
                    People
                </h1>
            </div>
            <!-- INFO START -->
            <div class="allInfoTravelWrapper">
                <div class="titleIntroductionWrapper">
                    <h1 class="titleFoodMain">
                        Do Son beach
                    </h1>
                </div> 
                <div id="DoSonBeachWrapper">
                    <div class="locationTitleWrapper">
                        <h1 class="locationTitleMain">
                            Location: 
                        </h1>
                        <p class="locationDetailMain">Do Son Tourist Area, Van Huong, Do Son, Hai Phong, Vietnam</p>
                    </div>
                    <div id="googleMapWrapperDoSon">
                        <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d29858.147847747812!2d106.77530875644953!3d20.699319529182965!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x314a6c608b8feabd%3A0x5bf0cbf27e7679c2!2zQsOjaSBiaeG7g24gxJDhu5MgU8ahbg!5e0!3m2!1svi!2s!4v1735183178118!5m2!1svi!2s" width="600" height="450" style="border:0;" allowfullscreen="true" loading="lazy" referrerpolicy="no-referrer-when-downgrade" id="googleMapMainDoSon"></iframe >
                    </div>
                    <div class="locationTitleWrapper">
                        <h1 class="locationTitleMain">
                            Time to travel: 
                        </h1>
                        <p class="locationDetailMain">May, June, July</p>
                    </div>
                    <div class="locationTitleWrapper">
                        <h1 class="locationTitleMain">
                            Cuisines:
                        </h1>
                    </div>
                </div>
                <div id="CuisinesDoSonImageIWrapper">
                    <div class="OnlyImageWrapper">
                        <img src="./image/NomSuaDoSon.jpg" class="OnlyImageMain">
                    </div>
                    <div class="OnlyImageWrapper">
                        <img src="./image/LauCuaDongNemCuaBe.jpg" class="OnlyImageMain">
                    </div>
                    <div class="OnlyImageWrapper">
                        <img src="./image/CaMoiMotNangDoSon.jpeg" class="OnlyImageMain">
                    </div>  
                    <div class="OnlyImageWrapper">
                        <img src="./image/HaiSanDoSon.jpg" class="OnlyImageMain">
                    </div>
                </div>
                <div class="infoCuisineDoSonWrapper">
                    <div class="infoDoSonWrapper">
                        <h1 class="infoDoSonMain">Do Son jellyfish salad</h1>
                    </div>
                    <div class="infoDoSonWrapper">
                        <h1 class="infoDoSonMain">Do Son crab hotpot, crab spring rolls</h1>
                    </div>
                    <div class="infoDoSonWrapper">
                        <h1 class="infoDoSonMain">Do Son sun-dried anchovies</h1>
                    </div>
                    <div class="infoDoSonWrapper">
                        <h1 class="infoDoSonMain">Do Son fresh seafood</h1>
                    </div>
                </div>
            </div>
            <div class="allInfoTravelWrapper">
                <div class="titleIntroductionWrapper">
                    <h1 class="titleFoodMain">
                        Cat Ba Island
                    </h1>
                </div> 
                <div id="DoSonBeachWrapper">
                    <div class="locationTitleWrapper">
                        <h1 class="locationTitleMain">
                            Location: 
                        </h1>
                        <p class="locationDetailMain">Hai Phong city, Vietnam</p>
                    </div>
                    <div id="googleMapWrapperDoSon">
                        <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d29858.147847747812!2d106.77530875644953!3d20.699319529182965!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x314a6c608b8feabd%3A0x5bf0cbf27e7679c2!2zQsOjaSBiaeG7g24gxJDhu5MgU8ahbg!5e0!3m2!1svi!2s!4v1735183178118!5m2!1svi!2s" width="600" height="450" style="border:0;" allowfullscreen="true" loading="lazy" referrerpolicy="no-referrer-when-downgrade" id="googleMapMainDoSon"></iframe >
                    </div>
                    <div class="locationTitleWrapper">
                        <h1 class="locationTitleMain">
                            Time to travel: 
                        </h1>
                        <p class="locationDetailMain">From April to October</p>
                    </div>
                    <div class="locationTitleWrapper">
                        <h1 class="locationTitleMain">
                            Cuisines:
                        </h1>
                    </div>
                </div>
            </div>
            <div class="allInfoTravelWrapper">

            </div>
            <div class="allInfoTravelWrapper">

            </div>
            <!-- INFO END -->
        </div>
    </div>
</body>
</html>